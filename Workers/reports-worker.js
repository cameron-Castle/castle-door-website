export default {
  async fetch(req, env, ctx) {
    ...
    // reports-worker only redirects portal traffic to the canonical portal origin.
    // If routing/bindings still send portal traffic here on the canonical host,
    // fail closed to prevent split ownership behavior.
    if (isPortalPath) {
      let target;
      try {
      ...
      if (target && target.origin !== url.origin) {
        console.log("[reports-portal-ownership] redirect", {
          fromHost: hostname,
      ...
      console.log("[reports-portal-ownership] reject", {
        host: hostname,
        path: pathname,
        method: req.method,
        portalOrigin,
      });
      return new Response("Portal routes are served by portal-worker", {
        status: 421,
        headers: {
    ...
    const isUidLookupDebug = String(env.DEBUG_UID_LOOKUP || "") === "1";
    const uidLookupDebug = (...args) => {
      if (isUidLookupDebug) console.log("[reports-uid-lookup-debug]", ...args);
    };
    ...
    async function getBusinessConfig(env, businessCode) {
    ...
    async function isSandboxBusiness(env, businessCode) {
      const cfg = await getBusinessConfig(env, businessCode);
    ...
    async function submitCtaRequest(env, ctx, req, payload, options = {}) {
      ...
      if (sandboxMode) {
        return {
          response: json({ ok: true, id, sandbox: true, simulated: true }),
          id,
        };
      }
      ...
      const sendEmail = async () => {
        try {
          ...
          <div style="font-family: system-ui, sans-serif; color: #222; line-height: 1.45;">
            <h2 style="margin-bottom: 0.3em;">${escHtml(kindLabel)} Request – ${escHtml(doorName)}</h2>
            <div style="font-size: 14px; color: #555;">${escHtml(bizName)} • ${escHtml(bldgName)}</div>
            <hr style="margin: 1em 0; border: none; border-top: 1px solid #ddd;" />
            <p>A ${escHtml(kindLabel.toLowerCase())} request was submitted for the door below:</p>
            <ul>
              <li><strong>Business:</strong> ${escHtml(bizName)} ${businessCode ? `(${escHtml(businessCode)})` : ""}</li>
              <li><strong>Building:</strong> ${escHtml(bldgName)} ${buildingCode ? `(${escHtml(buildingCode)})` : ""}</li>
              <li><strong>Door:</strong> ${escHtml(doorName)} ${doorSlug ? `(${escHtml(doorSlug)})` : ""}</li>
              ${statusText ? `<li><strong>Status:</strong> ${escHtml(statusText)}</li>` : ""}
              <li><strong>Requester:</strong> ${escHtml(safeRequester)} &lt;${escHtml(safeRequesterEmail)}&gt;</li>
            </ul>
            <h3 style="margin-top: 1em;">Notes</h3>
            <pre style="background: #f8f8f8; padding: 10px; border-radius: 6px; white-space: pre-wrap; font-family: system-ui, sans-serif;">${escHtml(safeNotes)}</pre>
            ${doorUrl ? `<p style="margin-top: 1.5em;"><a href="${escHtml(doorUrl)}" style="display: inline-block; padding: 10px 14px; background: #2b5cff; color: #fff; border-radius: 6px; text-decoration: none; font-weight: 600;">View Full Door Report</a></p>` : ""}
            <p style="margin-top: 1.5em; font-size: 12px; color: #999;">Request ID: ${escHtml(id)}</p>
          </div>
          `;
    ...
    const buildUidCandidates = (token) => {
      const raw = String(token || "").trim();
      if (!raw) return [];
      const sanitized = sanitizeUidToken(raw);
      const out = [
        raw,
        sanitized,
        raw.toLowerCase(),
    ...
    async function resolveMappingFromDoorRecords(kv, token) {
      const uidCandidates = buildUidCandidates(token);
      const safeCandidates = uidCandidates
        .map((v) => sanitizeUidToken(v))
        .filter((v, i, arr) => v && arr.indexOf(v) === i);

      // Fast path: UID-first door records (door:<uid>)
      for (const safeUid of safeCandidates) {
        const rec = await kv.get(`door:${safeUid}`, "json");
        if (!rec || typeof rec !== "object") continue;
        const businessCode = String(rec.businessCode || "").trim();
        const buildingCode = String(rec.buildingCode || "").trim();
        const doorSlug = String(rec.doorSlug || "").trim();
        if (businessCode && buildingCode && doorSlug) {
          return {
            mapping: { businessCode, buildingCode, doorSlug },
            matchedUid: safeUid,
            source: "door_uid_record",
          };
        }
      }

      // Slow path: scan door:<biz>:<building>:<slug> summaries for matching UID
      ...
      do {
        const listed = await kv.list({ prefix: "door:", cursor, limit: 200 });
        for (const entry of listed.keys || []) {
          ...
          const rec = await kv.get(keyName, "json");
          if (!rec || typeof rec !== "object") continue;
          const safeUid = sanitizeUidToken(rec.doorId || "");
          if (!safeSet.has(safeUid)) continue;
    ...
    const normalizeBuildingName = (value = "") => {
      ...
      if (!raw) return "Main";
      if (lowered === "0" || compact === "0") return "Main";
      if (lowered === "null" || lowered === "undefined") return "Main";

      const mainLike = new Set([
        "main",
        "mainn",
        "mian",
        "mani",
      ...
      if (mainLike.has(compact)) return "Main";
      return raw;
    };
    ...
    async function getOrCreateBuildingCode(kv, businessCode, buildingName) {
      const norm = normalizeBuildingName(buildingName).toLowerCase();
    ...
    const parseUploadFieldsFromBaseName = (baseName = "") => {
      ...
      if (doorStatusRaw) {
        ...
        else if (v === "fail" || v === "flagged") status = "Flagged";
      ...
      if (isUploadDebug) {
        console.log("[upload-status-map-helper]", {
          doorStatusRaw,
          triRaw,
          mappedStatus: status,
          baseName,
    ...
    const requireAdminCookie = () => {
      ...
      console.log("[reports-admin-auth-miss]", {
        host: hostname,
        path: pathname,
        query: url.search,
        adminBase,
...
<html>
  ...
  <body>
    ...
    if (req.method === "POST" && pathname === "/upload") {
      ...
      if (isUploadDebug) {
        console.log("[upload-debug:filename-base]", {
          rawName,
          baseName,
        });
      }
      ...
      let barcodeUid = "";
      let submittedAt = "";
      let doorStatusRaw = ""; // Pass / Conditional Pass / Flagged
      let constructionCompany = "";
      ...
      if (isUploadDebug) {
        console.log("[upload-debug:parsed-fields]", {
          business,
          doorStatusRaw,
          triRaw,
          building,
      ...
      if (isUploadDebug) {
        console.log("[upload-status-map-main]", {
          rawName,
          doorStatusRaw,
          triRaw,
          mappedStatus: status,
      ...
      uidLookupDebug("upload-identity", {
        doorIdRaw,
        doorId,
        normalizedUid,
        hasRealUid,
      ...
      if (isUploadDebug) {
        console.log("[upload-debug:identity-derived]", {
          normalizedUid,
          hasRealUid,
          doorIdRaw,
          doorId,
      ...
      const buildingForCode = normalizeBuildingName(building);
      ...
      if (isUploadDebug) {
        console.log("[upload-debug:kv-write-plan]", {
          aliasKey,
          uidKey,
          doorKey,
          mapping: { businessCode, buildingCode, doorSlug },
          summaryLastReportKey: summary.lastReportKey,
        });
      }
      ...
      await Promise.all([
        ...
        // Keep business visibility in admin in sync with upload success.
        ensureBizConfig(),
      ]);

      if (isUploadDebug) {
        try {
          const aliasReadBack = await env.REPORTS_KV.get(aliasKey, "json");
          const uidReadBack = await env.REPORTS_KV.get(uidKey, "json");
          console.log("[upload-debug:kv-write-confirmed]", {
            aliasKey,
            aliasReadBack,
            uidKey,
            uidReadBackExists: !!uidReadBack,
            uidReadBackDoorId: uidReadBack?.doorId || "",
            uidReadBackLastReportKey: uidReadBack?.lastReportKey || "",
          });
        } catch (err) {
          console.log("[upload-debug:kv-write-confirmed:error]", {
    ...
    if (req.method === "POST" && pathname === "/upload-admin-pdf") {
      ...
      let building = normalizeBuildingName(parsed.building || "");
      ...
      if (mapping) {
        ...
        if (summary && typeof summary === "object") {
          ...
          building = normalizeBuildingName(summary.building || building);
  ...
  <body>
    ...
    if (req.method === "GET" && pathname.startsWith("/r/")) {
      ...
      if (parts.length === 2) {
        ...
        const redirectFinal = new URL(`${url.origin}${redirectPath}`);
        if (mapping && (await isSandboxBusiness(env, mapping.businessCode))) {
          redirectFinal.searchParams.set("demo", "1");
        }
        const redirectUrl = redirectFinal.toString();
    ...
    if (req.method === "GET" && pathname.startsWith("/reports/")) {
      ...
      if (requestedViewer === "admin") {
        console.log("[reports-admin-viewer-check]", {
          host: hostname,
          path: pathname,
          query: url.search,
          hasAdminSession,
          cookieKeys: Object.keys(reportCookies || {}),
        });
      }
      ...
      // Shape B (legacy): /reports/:doorId
        ...
        const uidCandidates = buildUidCandidates(token);
        doorId = token;
        uidLookupDebug("legacy-route-request", {
          token,
          uidCandidates,
          path: pathname,
          query: url.search,
        ...
        for (const candidate of uidCandidates) {
          const hit = await getDoorIndexMapping(env.REPORTS_KV, candidate);
          if (hit) {
            map = hit;
        ...
        if (map) {
          ...
          const canonicalUrl = `${url.origin}${canonical}`;
          uidLookupDebug("legacy-route-redirect", {
            token,
            matchedUid,
            map,
            canonical,
        ...
        const recovered = await resolveMappingFromDoorRecords(env.REPORTS_KV, token);
        if (recovered && recovered.mapping) {
          map = recovered.mapping;
          matchedUid = recovered.matchedUid || sanitizeUidToken(token);
          try {
            await env.REPORTS_KV.put(`doorIndex:${matchedUid}`, JSON.stringify(map));
          } catch {}

          const canonical = buildCanonicalReportPath(map) + (url.search || "");
          const canonicalUrl = `${url.origin}${canonical}`;
          uidLookupDebug("legacy-route-recovered-redirect", {
            token,
            matchedUid,
      ...
      // Use sanitized doorId as folder prefix in R2; legacy route may need variant fallbacks.
      const safeDoor = sanitizeUidToken(doorId);
      const safeDoorCandidates = (() => {
        const base = parts.length === 2 ? buildUidCandidates(doorId) : [doorId];
        const out = base.map((v) => sanitizeUidToken(v));
        return out.filter((v, i, arr) => v && arr.indexOf(v) === i);
      })();
      ...
      try {
        if (!doorRec) {
          for (const candidateSafeDoor of safeDoorCandidates) {
            const rec = await env.REPORTS_KV.get(`door:${candidateSafeDoor}`, "json");
        ...
        for (const candidateSafeDoor of safeDoorCandidates) {
          const rawUidEvents = await env.REPORTS_KV.get(`uidEvents:${candidateSafeDoor}`, "json");
      ...
      if (parts.length === 2 && doorRec && typeof doorRec === "object") {
        if (!businessCode && doorRec.businessCode) businessCode = String(doorRec.businessCode);
        if (!buildingCode && doorRec.buildingCode) buildingCode = String(doorRec.buildingCode);
        if (!doorSlug && doorRec.doorSlug) doorSlug = String(doorRec.doorSlug);
        if (!business && doorRec.business) business = String(doorRec.business);
        if (!building && doorRec.building) building = String(doorRec.building);

        if (businessCode && buildingCode && doorSlug) {
          const canonicalPath = buildCanonicalReportPath({ businessCode, buildingCode, doorSlug });
          if (url.pathname !== canonicalPath) {
            const canonicalUrl = `${url.origin}${canonicalPath}${url.search || ""}`;
            return Response.redirect(canonicalUrl, 302);
          }
        }
      }
      ...
      if (uidHistory) {
        ...
        for (const candidateSafeDoor of safeDoorCandidates) {
          const list = await env.REPORTS_BUCKET.list({
            prefix: isAdminViewer ? `${candidateSafeDoor}/admin/` : `${candidateSafeDoor}/`,
          });

          const candidateObjects = (list.objects || [])
            .filter((o) => {
      ...
      if (parts.length === 2) {
        uidLookupDebug("legacy-route-result", {
          requestedDoorId: doorId,
          safeDoor,
          safeDoorCandidates,
          hasUidHistory: !!uidHistory,
          objectCount: Array.isArray(objects) ? objects.length : 0,
          viewer: isAdminViewer ? "admin" : "customer",
        });
      }
      ...
      const normalizeDisplayStatus = (s) => {
        ...
        if (!v) return "";
        if (v.toLowerCase() === "fail") return "Flagged";
...
`
      ...
      const isDemoSandbox = await isSandboxBusiness(env, businessCode);
      const demoQueryRequested = parseBooleanFlag(url.searchParams.get("demo"));
      const sandboxMode = isDemoSandbox || demoQueryRequested;
...
<html>
  ...
  <body>
    ...
    <p>If you should have access, please contact your administrator or maintenance manager.</p>
      ...
      const normalizedMetaBuilding = normalizeBuildingName(meta.building || "");

      const metaLine = [
        `Door: ${esc(displayLabel)}`,
        meta.doorId && `Door ID: ${esc(meta.doorId)}`,
        meta.business && `Business: ${esc(meta.business)}`,
        normalizedMetaBuilding && `Building: ${esc(normalizedMetaBuilding)}`,
        effectiveStatus && `Status: ${esc(effectiveStatus)}`,
        sandboxMode && `Mode: Demo sandbox`,
        `Viewer: ${sandboxMode ? "Customer report" : (isAdminViewer ? "Admin report" : "Customer report")}`,
      ]
        .filter(Boolean)
        .join(" • ");
      ...
      const viewerToggleHtml = hasAdminSession && !sandboxMode ? `
      ...
      for (const item of doorList.keys || []) {
        try {
          ...
          if (!bName) {
            const normalizedUploadedBuilding = normalizeBuildingName(d.building || "");
            bName =
              normalizedUploadedBuilding ||
      ...
      function renderDoorList(list) {
        ...
        return list
          .map((d) => {
            ...
            const statusClass =
              statusValue === "fail" || statusValue === "flagged" || statusValue === "needs repair"
                ? "status-flagged"
                : statusValue === "conditional pass" || statusValue === "conditional"
                  ? "status-conditional"
                  : statusValue === "pass"
                    ? "status-pass"
                    : "status-other";
      ...
      <head>
        ...
        <style>
          :root {
            --bg: ${isAdminViewer ? "#060b17" : "#eef3ff"};
            --panel: ${isAdminViewer ? "rgba(15,23,42,0.72)" : "rgba(255,255,255,0.88)"};
            --panel-solid: ${isAdminViewer ? "#0f172a" : "#ffffff"};
            --border: ${isAdminViewer ? "rgba(148,163,184,0.24)" : "rgba(37,99,235,0.16)"};
            --accent: ${isAdminViewer ? "#8b5cf6" : "#2563eb"};
            --accent-soft: ${isAdminViewer ? "rgba(139,92,246,0.18)" : "rgba(37,99,235,0.12)"};
            --accent-strong: ${isAdminViewer ? "#7c3aed" : "#1d4ed8"};
            --text-main: ${isAdminViewer ? "#e2e8f0" : "#0f172a"};
            --text-muted: ${isAdminViewer ? "#94a3b8" : "#64748b"};
            --shadow-soft: ${isAdminViewer ? "0 12px 32px rgba(2,6,23,0.45)" : "0 10px 30px rgba(37,99,235,0.12)"};
          ...
          * {
          ...
          body {
            ...
            font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI",
              sans-serif;
            background:
              radial-gradient(1200px 420px at 20% -10%, ${isAdminViewer ? "rgba(124,58,237,0.26)" : "rgba(59,130,246,0.2)"}, transparent 60%),
              radial-gradient(900px 320px at 90% -20%, ${isAdminViewer ? "rgba(59,130,246,0.2)" : "rgba(139,92,246,0.16)"}, transparent 60%),
              var(--bg);
            color: var(--text-main);
            height: 100vh;
            overflow: hidden;
          ...
          .btn-primary {
            ...
            color: #fff;
            box-shadow: 0 8px 20px ${isAdminViewer ? "rgba(124,58,237,0.32)" : "rgba(37,99,235,0.28)"};
          }
      
          .btn:hover {
            background: ${isAdminViewer ? "#1e293b" : "#f8fbff"};
          ...
          .sidebar-section {
            ...
            border-radius: 12px;
            padding: 0.45rem;
            background: ${isAdminViewer ? "rgba(15,23,42,0.5)" : "rgba(255,255,255,0.72)"};
          }
          ...
          .status-pill.status-other {
            color: var(--text-main);
            border-color: var(--border);
            background: ${isAdminViewer ? "rgba(148,163,184,0.14)" : "rgba(148,163,184,0.16)"};
          }
          ...
          /* default: desktop/tablet keep hamburger hidden */
          .viewer-nav-toggle {
            display: none;
          }
      ...
      <body>
        ...
        <div class="header-actions">
          ...
          ${sandboxMode ? '<div class="demo-banner">Demo sandbox mode: actions simulate success for this visitor only. No permanent writes or notifications are sent.</div>' : ''}
          <main class="content">
          <aside class="sidebar">
  ...
  <a href="${allTargetSlug && allTargetBuildingCode
    ...
    <span class="building-count">(${totalDoorsAll})</span>
        ...
        window.__DOOR_CTX = ${JSON.stringify({
          ...
          function sandboxCommentsKey(uid) {
            return "demoSandboxComments:" + String(uid || "").trim();
          }

          function getSandboxComments(uid) {
            try {
              var key = sandboxCommentsKey(uid);
          ...
          function putSandboxComments(uid, comments) {
            try {
              sessionStorage.setItem(
                sandboxCommentsKey(uid),
                JSON.stringify(Array.isArray(comments) ? comments : [])
              );
            } catch (_e) {}
          }

          function addSandboxComment(uid, comment) {
            var existing = getSandboxComments(uid);
            existing.unshift(comment);
            putSandboxComments(uid, existing.slice(0, 50));
          }

          async function loadApprovedComments() {
            var ctx = window.__DOOR_CTX || {};
            var uid = String(ctx.uid || "").trim();
            var sandboxMode = !!ctx.sandboxMode;
            var list = document.getElementById("customer-comments-list");
            if (!uid || !list) {
            ...
            try {
              ...
              if (sandboxMode) {
                comments = comments.concat(getSandboxComments(uid));
              ...
              var localOnly = sandboxMode ? getSandboxComments(uid) : [];
              if (localOnly.length) {
                if (openCustomerCommentsBtn) {
                  openCustomerCommentsBtn.hidden = false;
                  openCustomerCommentsBtn.textContent = "View comments (" + localOnly.length + ")";
                }
                if (quickOpenCustomerCommentsBtn) {
                  quickOpenCustomerCommentsBtn.hidden = false;
                  quickOpenCustomerCommentsBtn.textContent = "Comments (" + localOnly.length + ")";
                }
                renderApprovedComments(localOnly);
                return;
              }
              if (openCustomerCommentsBtn) openCustomerCommentsBtn.hidden = true;
              if (quickOpenCustomerCommentsBtn) quickOpenCustomerCommentsBtn.hidden = true;
          ...
          if (commentForm) {
            commentForm.addEventListener("submit", async function (e) {
              ...
              if (ctx.sandboxMode) {
                ...
                addSandboxComment(uid, {
                  commentId: "demo-" + Math.random().toString(36).slice(2, 10),
                  uid: uid,
                  message: message,
                ...
                if (commentStatus) {
                  commentStatus.textContent = "Comment saved in demo sandbox for this browser session.";
                }
                closeCustomerComment();
  ...
  return urls;
            ...
            function startCtaRequest(kind) {
              ...
              var notes =
                ...
                if (ctx.sandboxMode) {
                  var fakeId = "demo-" + Date.now() + "-" + Math.random().toString(36).slice(2, 8);
                  alert("Demo sandbox: request captured for this session only. ID: " + fakeId);
                  return;
                }
    ...
    if (req.method === "POST" && pathname === "/api/cta-request") {
      ...
      const sandboxMode = await isSandboxBusiness(env, slug(body.businessCode || ""));
      const result = await submitCtaRequest(env, ctx, req, body, { sandboxMode });
    ...
    if (req.method === "POST" && pathname === "/api/customer-comment") {
      ...
      const sandboxMode = await isSandboxBusiness(env, resolved.mapping.businessCode);
      if (sandboxMode) {
        ...
        return json(
          {
            ok: true,
            commentKey: `sandbox:${resolved.uid}:${createdAt}:${commentId}`,
            commentId,
            uid: resolved.uid,
            status: "approved",
            createdAt,
      ...
      let bizCfg = {};
      try {
        bizCfg = (await env.ENROLL_TOKENS.get(`bizcfg:${resolved.mapping.businessCode}`, "json")) || {};
      } catch {
        bizCfg = {};
      }
      const publicAutoApprove = bizCfg.public_comment_auto_approve === true;
      ...
      const status = (isPortalAuth || publicAutoApprove) ? "approved" : "pending";
      ...
      const record = {
        ...
        message,
        createdAt,
        moderatedAt: status === "approved" ? createdAt : null,
        moderationReason: isPortalAuth
          ? "portal-auth-auto-approved"
          : (publicAutoApprove ? "business-public-auto-approved" : null),
